...
export default function SettingsPage() {
  ...
  if (loading)
    return <Loader2 className="animate-spin text-navy/40" />;

  return (
    <div className="max-w-xl">
      <h1 className="font-heading text-2xl font-extrabold text-navy">Site Settings</h1>
      ...
      <div className="mt-6 space-y-4 rounded-2xl border border-navy/10 bg-white p-6 shadow-sm">
        ...
        <button
          onClick={save}
          disabled={saving}
          className="flex w-full items-center justify-center gap-2 rounded-xl bg-orange px-4 py-3 font-semibold text-white transition-colors hover:bg-orange-dark disabled:opacity-60"
        >
          {saving ? <Loader2 size={17} className="animate-spin" /> : saved ? <><Check size={17} /> Saved</> : "Save Settings"}
        </button>
      </div>
    </div>
  );
